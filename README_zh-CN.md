...
**macOS CoreML 缓存：** 编译后的模型保存在
`~/Library/Caches/RawAlchemy/coreml/`。缓存命名空间由 ONNX 模型及其引用的全部
外部权重文件内容、ONNX Runtime 和系统版本、架构以及会话配置（包括去马赛克
分块大小）共同决定。同一路径下替换模型或权重内容后，下次初始化会话时会使用
新的命名空间。每次初始化会话时都会重新计算模型和外部权重内容的哈希，不使用
路径、文件大小或时间戳作为跳过哈希的捷径。如果缓存准备、哈希计算或可写性检查失败，会记录
警告并继续使用不带缓存的 CoreML。旧命名空间不会自动删除；可在关闭应用后删除该缓存目录以释放空间，
下次启动时将重新编译模型。

**去马赛克加速降级：** RCD 或 X-Trans 加速会话在初始化（包括 CoreML 编译）
或推理阶段失败时，会记录警告并尝试改用 CPU。初始化降级成功后，对应模块在
本次进程内持续使用 CPU，切换分块大小也不会重复尝试失败的加速编译。
处理速度可能降低；CPU 自身失败仍会报错，不会无限重试。重启应用可重新尝试加速。
