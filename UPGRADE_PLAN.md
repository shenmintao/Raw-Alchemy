...
## Phase 7 — 交互性能与显存/内存治理（2026-07-02 诊断追加；已实施）

> 25-agent 对抗验证诊断产出，针对三个用户症状：批量加载卡顿、缩放卡顿、
> 被误判为"显存问题"的主机内存膨胀。任务全文与验收标准见
> `docs/PHASE7_PLAN.md`。T7.1-T7.8 已在 feat/phase7-perf 实施完毕并合入，
> 附带 13 项复查缺陷修复（并发锁、缓存命中状态同步、关闭路径线程治理等）。
> 自动化验收：158 项测试全绿；帧率/体感类验收项待 Windows 真机人工确认
> （清单见 docs/UPGRADE_STATUS.md 的 pre3 一节）。
